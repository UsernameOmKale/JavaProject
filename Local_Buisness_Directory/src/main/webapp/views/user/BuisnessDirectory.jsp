...
<body>
    ...
    <div class="main-content">
        ...
        <div class="table-container">
            ...
            <tr id="no-data-message" style="display:none;">
                <td colspan="6" style="text-align: center; color: black; font-weight: bold; padding: 20px;">Not More data found</td>
            </tr> -->
        </div>
    </div>
    ...
    %>
        <script>
            // SweetAlert message if dataExist is not null
            Swal.fire({
                title: 'Oops!',
    ...
    %>
        <script>
            Swal.fire({
                title: 'Ooppppp!',
                text: 'You Already Like This Post',
                icon: 'smile',
                confirmButtonText: 'OK'
            });
        </script>
    ...
    function searchTable() {
        ...
        let isDataFound = false; // Flag to check if any row matches the search

        for (let i = 1; i < rows.length; i++) { // Start from i=1 to skip the header row
            let cells = rows[i].getElementsByTagName("td");
            
            let businessCell = cells[1].textContent.toLowerCase(); // Business Name column
            let cityCell = cells[3].textContent.toLowerCase();    // City column (index 3)
            let addressCell = cells[4].textContent.toLowerCase(); // Address column (index 4)
            ...
            if (isBusinessMatch && isCityMatch && isAddressMatch) {
                rows[i].style.display = "";
                isDataFound = true; // Data found, show the table rows
            } else {
                rows[i].style.display = "none";
            }
        }

        // If no data is found, display "No data found" message
        if (!isDataFound) {
            noDataMessage.style.display = "table-row"; // Make the No Data Found row visible
        } else {
            noDataMessage.style.display = "none"; // Hide the No Data Found row
        }
    }
    </script>
